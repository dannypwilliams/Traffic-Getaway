...
- Full recorded playtest current counts: planned tests 73, passed 20, failed 3, partial 8, not tested 42, valid active-input runs 12, invalid runs 6.
...
- Full recorded playtest capture-tooling note: `--control-preference` now seeds the saved control mode for future control-mode evidence. A later New York start-gate probe showed Los Angeles on screen despite `ny_01` defaults, so no city-2 evidence was accepted from that attempt and the leftover probe state was cleaned up.
- Logs:
