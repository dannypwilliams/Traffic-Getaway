...
- Full recorded playtest current counts: planned tests 73, passed 17, failed 1, partial 2, not tested 53, valid active-input runs 11, invalid runs 2.
...
| Full recorded playtest | In progress | Artifact scaffold exists; automated validation passed; LA Starter Compact iPhone 17e slice has five complete-evidence active runs plus one supplemental active run; LA Starter Bike has three complete-evidence active runs; failure/retry functional session is recorded; iPhone 17 Pro Dynamic Island active sample is recorded but failed the top-HUD safe-area check and exposed a vehicle identity mismatch; opening LA slices are 0% complete in counted active play |
