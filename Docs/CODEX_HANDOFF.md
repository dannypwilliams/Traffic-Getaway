...
- Full recorded playtest current counts: planned tests 73, passed 19, failed 1, partial 4, not tested 49, valid active-input runs 11, invalid runs 3.
...
- P1 milestone blocker: Full recorded playtest is incomplete. Current full-playtest artifact has completed only the Los Angeles Starter Compact and Starter Bike iPhone 17e slices, tutorial completion, failure/retry, and one Dynamic Island-class safe-area/control sample; New York, Miami, active progression, pause/backgrounding, and many required videos are still not tested.
...
Continue `PlaytestArtifacts/2026-06-23-full-recorded-playtest/`: capture a true active Los Angeles progression attempt next, using an input path that reliably records lane changes after the tutorial-to-run transition. Separately investigate the Dynamic Island top-HUD overlap and iPhone 17 Pro vehicle identity mismatch before relying on more iPhone 17 Pro vehicle-specific rows, then continue New York/Miami starter-vehicle coverage before retuning Sunset Merge.
