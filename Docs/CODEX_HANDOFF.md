...
- Full recorded playtest current counts: planned tests 73, passed 20, failed 3, partial 10, not tested 40, valid active-input runs 12, invalid runs 6.
...
- Full recorded playtest city start-gate evidence: New York `ny_01` and Miami `mia_01` start screens are recorded with full-screen video, screenshot, defaults probe logs, and cleanup proof. These artifacts are start-screen-only evidence and do not count as active-input city runs, progression, balance, fairness, stress, or completion coverage.
- Logs:
