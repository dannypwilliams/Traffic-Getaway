...
- Full recorded playtest current counts: planned tests 73, passed 20, failed 1, partial 4, not tested 48, valid active-input runs 12, invalid runs 3.
...
Continue `PlaytestArtifacts/2026-06-23-full-recorded-playtest/`: capture a longer active Los Angeles progression attempt next, using the verified coordinate tap path and aiming to reach the exit phase or complete Sunset Merge. Separately investigate the Dynamic Island top-HUD overlap and iPhone 17 Pro vehicle identity mismatch before relying on more iPhone 17 Pro vehicle-specific rows, then continue New York/Miami starter-vehicle coverage before retuning Sunset Merge.
