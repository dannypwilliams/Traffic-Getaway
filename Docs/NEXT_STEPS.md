...
1. Capture active-steering manual iPhone 17e runs; passive no-input coverage now exists and is red.
2. Capture active-steering manual Dynamic Island-class runs; passive no-input iPhone 17 Pro coverage now exists and is red.
3. Fix passive-driver outcome readability so no-input play tends toward police capture pressure instead of early traffic/roadblock crash.
