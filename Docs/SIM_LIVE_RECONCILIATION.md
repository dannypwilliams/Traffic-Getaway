...
Partial. Core stress gates are fixed, debug live gameplay telemetry works, and iPhone 17e autoplay matrices now exist. The latest decision matrix narrows the GameSim-vs-live gap: applied live moves usually land on the intended target, but the live decision/state model still diverges from GameSim and crashes before the exit.
...
|---|---:|---:|---:|---|---|
| Exit activation time | 42s target from config | Not reached in 5/5 decision-matrix autoplay runs | Live ends before exit | Live decision/state model crashes early | Reconcile decision timing before tuning |
| First crash time | p50 36.8s | median 5.2s / avg 6.3s | Live autoplay much earlier | 36/207 target-policy mismatches; only 2/18 move-target and applied-slot mismatches | Reconcile stay-vs-move and reachable-slot state |
| Traffic waves before failure | Stress: 0 impossible / 160,000 | avg 6.2 waves before crash | Live ends early | Live frames often report no reachable safe slot while GameSim survives | Inspect active roster and timing at no-reachable/collision frames |
| Near misses | 35.3/run | 0.6/run | Live much lower | Short live runs plus different decision/timing model | Do not tune rewards from this sample |
...
- Compare no-reachable-safe-slot and collision frames against GameSim's per-wave state for the same seeds.
- Decide whether GameSim should model live movement cadence and active traffic lifetime more directly, or whether live safety selection should use a longer route horizon.
