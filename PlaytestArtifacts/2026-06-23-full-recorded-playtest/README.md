...
- Every non-`la_01` Starter Compact level now has configured start-gate-only video/screenshot evidence under `videos/city-*/*starter-compact_start-gate.mp4` and `screenshots/city-*/*starter-compact_start-gate.png`, with probe logs and debug-default cleanup logs. The start screen displays city and vehicle, not the exact level name, so level-specific evidence relies on the saved debug-default probe logs. These are start-screen-only artifacts and do not count as active-input runs, city progression, balance, fairness, stress, or completion evidence.
- A continuation probe attempted to resume New York active-run capture with `scripts/capture_live_telemetry.py --manual --wait-for-start-tap`, but the Mac host session was on the lock screen and no Simulator click/steering input path was available. Evidence is retained under `logs/manual-input-blocker/`. This does not count as a run or a planned-test completion.
